...
const okClass =
  "mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-xs sm:text-sm border-slate-300 rounded-md";
type InputProps = {
    ...
    <>
            ...
            {...register("phone", {
              ...
              onChange: (event) => {
                return (
                  ...
                  setValue(
                    "phone",
                    formatPhoneNumberToNationalUSAformat(event.target.value),
                    { shouldValidate: true }
                  )
                );
              },
      ...
      {errors[name]?.type === "pattern" && (
        <p className="mt-2 text-sm text-red-600">
          El numero de telefono no es correcto
        </p>
      )}
    </>
  );
